...
class ExcelInfo():
    def __init__(self, nazev, index, excel_list):
        ...
        self._datum = str(self._datum_p.split(".")[2]) + "-" + str(self._datum_p.split(".")[1]) + "-" + str(
            self._datum_p.split(".")[0])
...
class ExportXML:
    ...
    def export_prijata_xml(self):
        ...
        for row in file_temp:
            zmena_dict = {
                "<typ:numberRequested>pohoda_facture_number</typ:numberRequested>": "<typ:numberRequested>" + str(
                    self._poc_cislo_faktury) + "</typ:numberRequested>",
                "<inv:symVar>facture_number</inv:symVar>": "<inv:symVar>" + re.sub('\D', '',
                                                                                   str(self._cislo_faktury)) + "</inv:symVar>",
                "<inv:originalDocument>facture_number</inv:originalDocument>": "<inv:originalDocument>" + str(
                    self._cislo_faktury) + "</inv:originalDocument>",
                ...
                "<typ:accountNo>V_acount_number</typ:accountNo>": "<typ:accountNo>" + str(
                    self._V_acount_number) + "</typ:accountNo>",
                "<typ:bankCode>V_bank_code</typ:bankCode>": "<typ:bankCode>" + str(
                    self._V_bank_code) + "</typ:bankCode>",
                "<inv:note>V_note</inv:note>": "<inv:note>" + str(self._V_note) + "</inv:note>",
                "<typ:amountHome>V_amount</typ:amountHome>": "<typ:amountHome>" + str(
                    self._castka) + "</typ:amountHome>",
    ...
    def export_vydana_xml(self):
        ...
        for row in file_temp:
            zmena_dict = {
                "<typ:numberRequested>pohoda_facture_number</typ:numberRequested>": "<typ:numberRequested>" + str(
                    self._poc_cislo_faktury) + "</typ:numberRequested>",
                "<inv:symVar>pohoda_facture_number</inv:symVar>": "<inv:symVar>" + str(
                    self._poc_cislo_faktury) + "</inv:symVar>",
                "<inv:date>datum</inv:date>": "<inv:date>" + str(self._datum) + "</inv:date>",
                "<inv:dateTax>datum</inv:dateTax>": "<inv:dateTax>" + str(self._datum) + "</inv:dateTax>",
                "<inv:dateDue>datum</inv:dateDue>": "<inv:dateDue>" + str(self._datum) + "</inv:dateDue>",
                "<inv:numberKHDPH>facture_number</inv:numberKHDPH>": "<inv:numberKHDPH>" + str(
                    self._cislo_faktury) + "</inv:numberKHDPH>",
                "<typ:company>C_firm</typ:company>": "<typ:company>" + str(self._C_company) + "</typ:company>",
                "<typ:city>C_city</typ:city>": "<typ:city>" + str(self._C_city) + "</typ:city>",
                "<typ:street>C_street</typ:street>": "<typ:street>" + str(self._C_street) + "</typ:street>",
                "<typ:zip>C_zip</typ:zip>": "<typ:zip>" + str(self._C_zip) + "</typ:zip>",
                ...
                "<typ:priceNone>V_price</typ:priceNone>": "<typ:priceNone>" + str(
                    -1 * (self._castka)) + "</typ:priceNone>"
            }
            if row.strip() in zmena_dict:
                novy_radek = row.replace(row.strip(), zmena_dict[row.strip()])
                facture_file.write(novy_radek)
...
def start_info(facture_name):
    ...
    facture_file.write(
        '<dat:dataPack version="2.0" id="Usr01" ico="682134908" key="c63711ca-75a9-4c76-8fbf-f35940263e5a" programVersion="12804.4 (7.7.2021)" application="Transformace" note="Uživatelský export" xmlns:dat="http://www.stormware.cz/schema/version_2/data.xsd">\n')
    facture_file.close()
...
def excel_info(facture_name, excel_list):
    ...
    index_konce = excel.iloc[:, 0].dropna().shape[0]
    return (index_konce)
...
def prijate_faktury(excel_list, poc_cislo_faktury_prijate, facture_name):
    ...
    for i in p_bar:
        ...
        poc_cislo_faktury_prijate = str(poc_cislo_faktury_prijate.split("F")[0]) + "F" + str(
            format_d.format(int(poc_cislo_faktury_prijate.split("F")[1]) + 1))
...
def get_facture_name():
    facture_list = []
    path = os.getcwd()
    facture_names = os.listdir(path + "\FACTURES")
    for facture_name in facture_names:
        if facture_name.split(".")[-1] == "xlsx":
            facture_list.append(facture_name.split(".")[0])
    if len(facture_list) == 1:
        facture = facture_list[0]
    elif len(facture_list) >= 2:
        raise FileExistsError(f"You have more than 2 .xlsx files: {facture_list}")
    else:
        raise FileNotFoundError(f"You have 0 .xlsx files: {facture_list}")
    return facture


def main():
    facture_name = get_facture_name()
    #   PRIJATE FAKTURY   #
    # Stačí poc_cislo_faktury_prijate="", pohoda si to doplní sama #
    prijate_faktury(excel_list="EK RCH", poc_cislo_faktury_prijate="23F0001", facture_name=facture_name)
    #   VYDANE FAKTURY   #
    # musis zadat poc_cislo_faktury_vydana (+1)!!!!!!!!!!!!!!!!!!!! #
    vydane_faktury(excel_list="VK RCH", poc_cislo_faktury_vydana="230001", facture_name=facture_name)
